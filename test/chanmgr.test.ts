...
describe('ChannelManager', () => {
  it('can priv', async () => {
    ...
    expect(await echan.list({channel_id: group.id, privkey: group.privkey})).toHaveLength(0);
  ...
  it('can pub', async () => {
    ...
    console.log('sending to channel', group.id);
    const ev = await echan.send({channel_id: group.id, content: 'hello world', is_private: false});
    console.log('sent event', ev);
  ...
  it('can meta priv', async () => {
    ...
    await echan.setMeta(group.id, true, {name: "bob2", about: "bob2", picture: "bob2"})
    expect(await echan.getMeta(group.id, group.privkey)).toEqual({name: "bob2", about: "bob2", picture: "bob2", is_private: true})
    await pool.close();
  });
  ...
  it('can meta pub', async () => {
    ...
    await echan.setMeta(group.id, false, {name: "bob2", about: "bob2", picture: "bob2"})
    const {name, about, picture, is_private} = await echan.getMeta(group.id)
    expect({name, about, picture, is_private}).toEqual({name: "bob2", about: "bob2", picture: "bob2", is_private: false})
    await pool.close();
  });
